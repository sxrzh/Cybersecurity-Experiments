...
def get_user_by_token(token):
    ...
    user_id = -1
    if token:
        match = db.cursor().execute('SELECT * FROM tokens WHERE token = ?', [token]).fetchall()      # fix 1
        if len(match) > 0:
            user_id = match[0][1]
    return user_id
...
def login():
    ...
    match = db.cursor().execute('SELECT id, username, password_hash FROM users '
                                'WHERE username = ?', [username]).fetchall()  # fix 2
    if len(match) == 0:
        ...
        response.set_cookie("token", generate_token(info[0]), max_age = 60*24*3600, samesite = 'Strict')
        return response
...
def register():
    ...
    id = cur.lastrowid
    response = make_response(redirect("/"))
    response.set_cookie("token", generate_token(id), max_age = 60*24*3600, samesite = 'Strict')
    return response
